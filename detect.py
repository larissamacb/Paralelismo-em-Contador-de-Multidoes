...
def run_detect_on_dir(weights, source, classes, additional_args):
    command = f'"{sys.executable}" detect_single.py --weights {weights} --source {source} --classes {" ".join(map(str, classes))} {additional_args}'
    os.system(command)


def split_images(source_folder, num_parts):
    image_paths = sorted(Path(source_folder).glob('image_*.png'))
        ...
        if k not in ['weights', 'source', 'classes'] and v:
            ...
            if arg_name in valid_args:
                if isinstance(v, bool):
                    additional_args.append(f"--{arg_name}")
                else:
                    additional_args.append(f"--{arg_name} {v}")
